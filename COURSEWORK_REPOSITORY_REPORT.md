...
## 7. Recommended Repository Cleanup

For a public GitHub repository, the following cleanup would improve the repository without changing the coursework:

### Keep

- source `.c` and `.cpp` files;
- assignment-level READMEs;
- root README;
...
These are compiled binaries and can be regenerated from source.

### Keep only if redistribution is permitted

- assignment PDFs.

The PDFs are currently present in the archive. Whether they should remain in a public repository depends on the course's redistribution/copyright rules.

### Do not treat as coursework

- `.git/`

This is Git repository metadata rather than part of the assignment work.

---

## 8. Final Assessment

The repository itself is structurally sound and the coursework is accurately represented once the documentation is corrected.

The important corrections are:

1. use `Assignment_0` through `Assignment_9`;
2. include all ten assignments in the root README;
3. treat `Assignment_8` as EE1102 coursework as requested;
4. correct `pointer_strlen.c` to `pointer_strlen.cpp`;
5. acknowledge that the assignment PDFs are present;
6. state that `employee_data.txt` is required but missing;
7. identify `.exe` files as generated binaries;
8. describe the repository as a collection of EE1102 coursework rather than as a single software project.

With these corrections, the root README accurately reflects the supplied repository structure and contents.
